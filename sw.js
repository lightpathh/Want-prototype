      ...
      .then((cache) =>
        Promise.all(
          PRECACHE_URLS.map((url) =>
            cache.add(url).catch(() => console.warn('[SW] キャッシュ失敗（無視）:', url))
          )
        )
      )
  ...
  event.waitUntil(
    caches.keys()
      .then((keys) => {
        ...
        ).then(() => oldKeys.length > 0); // ← 削除があったか（=更新か）を返す
      })
      .then((wasUpdated) => self.clients.claim().then(() => wasUpdated))
      .then((wasUpdated) => {
        if (!wasUpdated) return; // ← 初回インストール時は通知しない
